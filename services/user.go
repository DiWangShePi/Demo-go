...
func UserUpdateUser(c *gin.Context) {
	...
	if !exists {
		c.JSON(200, gin.H{
			"status":  "Error",
			"message": "Can not found claims",
		})
		return
	} else {
		yourClaims := claims.(*utils.CustomClaims)
		userName := yourClaims.ID
		if userName != StudentId {
			c.JSON(200, gin.H{
				"status":  "Error",
				"message": "User can only change their own information",
			})
			return
		}
	}
...
func UserFindUser(c *gin.Context) {
	...
	if !exists {
		...
		if userName != StudentId {
			c.JSON(200, gin.H{
				"status":  "Error",
				"message": "User can only check their own information",
			})
			return
		}
	}
